...
class CWebUser extends CApplicationComponent implements IWebUser {
	 ...
	 * This is used by {@link getName} when the current user is a guest (not
	 * authenticated).
	 */
	public $guestName = 'Guest';
	/**
	 * @var string|array the URL for login. If using array, the first element should
	 * be
	 * the route to the login action, and the rest name-value pairs are GET
	 * parameters
	 * to construct the login URL (e.g. array('/site/login')). If this property is
	 * null,
	...
	/**
	 * @var array the property values (in name-value pairs) used to initialize the
	 * identity cookie.
	 ...
	 * If this property is not set, the user will be logged out after the current
	 * session expires
	...
	public $authTimeout;
	/**
	 * @var boolean whether to automatically renew the identity cookie each time a
	 * page is requested.
	 * Defaults to false. This property is effective only when {@link allowAutoLogin}
	 * is true.
	 * When this is false, the identity cookie will expire after the specified
	 * duration since the user
	 * is initially logged in. When this is true, the identity cookie will expire
	 ...
	 * Defaults to true, meaning flash messages will be valid only in the current and
	 * the next requests.
	 * If this is set false, you will be responsible for ensuring a flash message is
	 * deleted after usage.
	 * (This can be achieved by calling {@link getFlash} with the 3rd parameter being
	 * true).
	 * @since 1.1.7
	 */
	public $autoUpdateFlash = true;
	/**
	 * @var string value that will be echoed in case that user session has expired
	 * during an ajax call.
	 * When a request is made and user session has expired, {@link loginRequired}
	 * redirects to {@link loginUrl} for login.
	 * If that happens during an ajax call, the complete HTML login page is returned
	 * as the result of that ajax call. That could be
	 * a problem if the ajax call expects the result to be a json array or a
	 * predefined string, as the login page is ignored in that case.
	 * To solve this, set this property to the desired return value.
	 *
	 * If this property is set, this value will be returned as the result of the ajax
	 * call in case that the user session has expired.
	 * @since 1.1.9
	 * @see loginRequired
	 */
	...
	/**
	 * PHP magic method.
	 * This method is overriden so that persistent states can also be checked for
	 * null value.
	 * @param string $name property name
	 * @return boolean
	 */
	 ...
	 *
	 * @param IUserIdentity $identity the user identity (which should already be
	 * authenticated)
	 * @param integer $duration number of seconds that the user can remain in
	 * logged-in status. Defaults to 0, meaning login till the user closes the
	 * browser.
	 * If greater than 0, cookie-based login will be used. In this case, {@link
	 * allowAutoLogin}
	 ...
	 * @param boolean $destroySession whether to destroy the whole session. Defaults
	 * to true. If false,
	 * then {@link clearStates} will be called, which removes only the data stored
	 * via {@link setState}.
	 */
	 ...
	 * @return mixed the unique identifier for the user. If null, it means the user
	 * is a guest.
	 */
	...
	/**
	 * @param mixed $value the unique identifier for the user. If null, it means the
	 * user is a guest.
	 */
	 ...
	 * @param string $defaultUrl the default return URL in case it was not set
	 * previously. If this is null,
	...
	/**
	 * @param string $value the URL that the user should be redirected to after
	 * login.
	 */
	 ...
	 * @param array $states a set of name-value pairs that are provided by the user
	 * identity.
	 ...
	 * This method is used when automatic login ({@link allowAutoLogin}) is enabled.
	 * This method saves user ID, username, other identity states and a validation
	 * key to cookie.
	 * These information are used to do authentication next time when user visits the
	 * application.
	 * @param integer $duration number of seconds that the user can remain in
	 * logged-in status. Defaults to 0, meaning login till the user closes the
	 * browser.
	...
	/**
	 * @return string a prefix for the name of the session variables storing user
	 * session data.
	 */
	...
	/**
	 * @param string $value a prefix for the name of the session variables storing
	 * user session data.
	 */
	 ...
	 * @param mixed $defaultValue default value. If $value===$defaultValue, the
	 * variable will be
	 ...
	 * @param boolean $delete whether to delete the flash messages after calling this
	 * method.
	 ...
	 * @param mixed $defaultValue value to be returned if the flash message is not
	 * available.
	 * @param boolean $delete whether to delete this flash message after accessing
	 * it.
	 * Defaults to true.
	 ...
	 * @param mixed $defaultValue if this value is the same as the flash message, the
	 * flash message
	 * will be removed. (Therefore, you can use setFlash('key',null) to remove a
	 * flash message.)
	 */
	 ...
	 * @param array $params name-value pairs that would be passed to business rules
	 * associated
	 * with the tasks and roles assigned to the user.
	 * Since version 1.1.11 a param with name 'userId' is added to this array, which
	 * holds the value of
	 * {@link getId()} when {@link CDbAuthManager} or {@link CPhpAuthManager} is
	 * used.
	 * @param boolean $allowCaching whether to allow caching the result of access
	 * check.
	 * When this parameter
	 * is true (default), if the access check of an operation was performed before,
	 * its result will be directly returned when calling this method to check the
	 * same operation.
	 * If this parameter is false, this method will always call {@link
	 * CAuthManager::checkAccess}
